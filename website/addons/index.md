...
| [`addon-compat`](/addons/compat)                   | `.compat`             | native-style `setTimeout`/`setInterval`/`setRecurring` and `performance` signatures, for migrating incrementally | `WithCompatApi`         | [npm](https://www.npmjs.com/package/@time-provider/addon-compat)          |
