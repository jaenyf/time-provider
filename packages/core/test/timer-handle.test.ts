...
import { IRuntime, ITimerHandle, TIMER_KIND_TIMEOUT } from "../src/types/types.ts";
...
describe("timer-handle", () => {
  ...
  describe("dispose", () => {
    test("calling explicit dispose multiple times does not throw", () => {
      const sut = new TimerHandle(TIMER_KIND_TIMEOUT, createFakeRuntime(), undefined);
      sut.dispose();
      sut.dispose();
      sut.dispose();
    ...
    test("using implicit dispose makes instance disposed", () => {
      let runtimeRef: ITimerHandle | undefined = undefined;
      {
        using sut = new TimerHandle(TIMER_KIND_TIMEOUT, createFakeRuntime(), undefined);
        runtimeRef = sut;
      }
