...
/*
 * request() registers via the runtime's specific()/takeOutSpecificCallbacks() capability, and
 * drain() retrieves via the same tag - so this fake only needs to model those two faithfully:
 * registering never fires in-line (a far-future delay), and takeOutSpecificCallbacks() removes
 * and returns callbacks for a given tag, oldest first, leaving entries under other tags
 * untouched. dispose()ing a handle removes its entry immediately, mirroring core's real
 * tag-list unlink-on-dispose behavior.
 */
function fakeRuntime(): IDeterministicRuntime<unknown> & { registeredCount: () => number } {
  ...
  return {
    ...
    specific(
      ...
      expect(toDuration(initialDelay)).toBeGreaterThan(0);
...
describe("DeterministicIdleScheduler", () => {
  ...
  describe("request", () => {
    ...
    test("registers a real entry with the runtime immediately", () => {
      using sut = new DeterministicIdleScheduler();
