...
import { toDuration, type IAddon, type IDurationSpec, type IRuntime } from "@time-provider/core";
 ...
 * taggedTimers at request time), so a minimal fake satisfies it for a focused unit test without
 * needing a real plugin/runtime - the cast is safe because these tests never exercise anything
 * else on the fake runtime.
 */
function fakeDeterministicRuntime(): {
  ...
  const taggedTimers = {
    register(tag: unknown, delay: IDurationSpec, callback: () => void) {
...
describe("idleAddon (deterministic)", () => {
  ...
  test("request() registers a real entry with the runtime's taggedTimers immediately", () => {
