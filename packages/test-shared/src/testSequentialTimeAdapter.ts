...
export function testSequentialTimeAdapter<TDate>(
  ...
  describe("createManualTimeAdapter", () => {
    ...
    test("can construct an object with a TDate", () => {
      plugin.createSequentialTimeAdapter([createDate("2026-01-01T00:00:00.000Z")]);
    });
  });
  ...
  describe("ManualTimeAdapter", () => {
    describe("localNow", () => {
      ...
      test("returns first added value", () => {
        const sut = createSequentialTimeAdapter();
        expect(sut.localNow()).toEqual(createDate("2026-01-01T00:00:01.000Z"));
      });
      test("multiple calls returns sequentially defined times", () => {
        const sut = createSequentialTimeAdapter();
        expect(sut.localNow()).toEqual(createDate("2026-01-01T00:00:01.000Z"));
        expect(sut.localNow()).toEqual(createDate("2026-01-01T00:00:02.000Z"));
        expect(sut.localNow()).toEqual(createDate("2026-01-01T00:00:03.000Z"));
      });
    });
    ...
    describe("utcNow", () => {
      ...
      test("multiple calls returns sequentially defined times", () => {
        const sut = createSequentialTimeAdapter();
        expect(sut.utcNow()).toEqual(createDate("2026-01-01T00:00:01.000Z"));
        expect(sut.utcNow()).toEqual(createDate("2026-01-01T00:00:02.000Z"));
        expect(sut.utcNow()).toEqual(createDate("2026-01-01T00:00:03.000Z"));
      });
    });
