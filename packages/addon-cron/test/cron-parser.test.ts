...
import { DefaultCalendarAdapter, type ITimeConverter } from "@time-provider/core";
import {
 ...
 * stand in for TDate here) paired with the shared default calendar adapter reproduces the
...
describe("parseCronExpression", () => {
  ...
  test("throws when the expression does not have exactly 5 fields", () => {
    expect(() => parseCronExpression("* * * *", adapter)).toThrow(/expected 5 fields/);
  });

  test("throws for a value out of range (too high)", () => {
    expect(() => parseCronExpression("60 * * * *", adapter)).toThrow(/out of range/);
  });

  test("throws for a value out of range (too low)", () => {
    expect(() => parseCronExpression("* * 0 * *", adapter)).toThrow(/out of range/);
  });

  test("throws for a non-numeric, non-name token", () => {
    expect(() => parseCronExpression("abc * * * *", adapter)).toThrow(/not a valid value/);
  });

  test.each(["0x1E", "1e1", "0o17", "0b101", "1.5"])(
    "throws for a JS numeric-literal-syntax value that isn't a plain decimal integer (%s)",
    (token) => {
      expect(() => parseCronExpression(`${token} * * * *`, adapter)).toThrow(/not a valid value/);
    },
  );

  test("throws for a JS numeric-literal-syntax step, same as a value", () => {
    expect(() => parseCronExpression("*/0x2 * * * *", adapter)).toThrow(/not a valid step/);
  });

  test("throws for an unrecognized name", () => {
    expect(() => parseCronExpression("0 0 1 FOO *", adapter)).toThrow(/not a valid value/);
  });

  test("throws for an empty token", () => {
    expect(() => parseCronExpression("1, * * * *", adapter)).toThrow(/not a valid value/);
  });

  test("throws for a range given in the wrong order", () => {
    expect(() => parseCronExpression("10-5 * * * *", adapter)).toThrow(/out of order/);
  });

  test("throws for a non-positive step", () => {
    expect(() => parseCronExpression("*/0 * * * *", adapter)).toThrow(/not a valid step/);
  });

  test("throws for a non-integer step", () => {
    expect(() => parseCronExpression("*/abc * * * *", adapter)).toThrow(/not a valid step/);
  });
  ...
  describe("a malformed part is rejected rather than silently truncated", () => {
    test.each(["*/2/3", "5/2/3", "1-10/2/3"])("throws for a second step in %s", (part) => {
      ...
      expect(() => parseCronExpression(`${part} * * * *`, adapter)).toThrow(
    ...
    test("a single step and a single range are still accepted", () => {
      expect([...parseCronExpression("0-10/5 * * * *", adapter).minute.values]).toEqual([0, 5, 10]);
    });
  });
});
...
describe("computeNextOccurrence", () => {
  ...
  test("wildcard expression matches the very next minute", () => {
    const parsed = parseCronExpression("* * * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 10, 30, 15);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 31, 0));
  });

  test("fixed time-of-day, later the same day", () => {
    const parsed = parseCronExpression("30 9 * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 8, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 9, 30, 0));
  });

  test("fixed time-of-day, already passed today rolls over to tomorrow", () => {
    const parsed = parseCronExpression("30 9 * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 10, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 2, 9, 30, 0));
  });

  test("minute list", () => {
    const parsed = parseCronExpression("0,15,30,45 * * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 10, 20, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 30, 0));
  });

  test("minute wildcard step", () => {
    const parsed = parseCronExpression("*/15 * * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 10, 5, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 15, 0));
  });

  test("minute range step", () => {
    const parsed = parseCronExpression("0-30/10 * * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 10, 5, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 10, 0));
  });

  test("hour range with step", () => {
    const parsed = parseCronExpression("0 8-18/2 * * *", adapter);
    const from = Date.UTC(2024, 0, 1, 9, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 0, 0));
  });
  ...
  test("day-of-week only (dayOfMonth wildcard): weekday range, rolls over the weekend", () => {
    ...
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 8, 9, 0, 0));
  });

  test("day-of-month only (dayOfWeek wildcard): monthly on a fixed day", () => {
    const parsed = parseCronExpression("0 0 15 * *", adapter);
    const from = Date.UTC(2024, 0, 20, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 1, 15, 0, 0, 0));
  });

  test("month restriction, by name list", () => {
    const parsed = parseCronExpression("0 0 1 JAN,JUL *", adapter);
    const from = Date.UTC(2024, 2, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 6, 1, 0, 0, 0));
  });
  ...
  test("day-of-week alias 7 (Sunday)", () => {
    ...
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 7, 0, 0, 0));
  });

  test("POSIX OR-quirk: both day-of-month and day-of-week restricted matches on either", () => {
    const parsed = parseCronExpression("0 0 1,15 * MON", adapter);
    // 2024-01-02 is a Tuesday; the nearer of "next Monday" (01-08) and "the 15th" is 01-08.
    const from = Date.UTC(2024, 0, 2, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 8, 0, 0, 0));
  });

  test("throws when the expression can never match", () => {
    const parsed = parseCronExpression("0 0 31 2 *", adapter);
    expect(() => computeNextOccurrence(parsed, Date.UTC(2024, 0, 1), utc, adapter)).toThrow(
      /No matching cron occurrence found/,
    );
  });

  test("resolves a leap-day-only expression across a century-boundary gap (up to 8 years)", () => {
    const parsed = parseCronExpression("0 0 29 2 *", adapter);
    ...
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2104, 1, 29, 0, 0, 0));
  });
  ...
  describe("timezone handling", () => {
    test("resolves a fixed local time-of-day against a non-UTC offset", () => {
      ...
      expect(computeNextOccurrence(parsed, from, "Europe/Paris", adapter)).toBe(
    ...
    test("a wall-clock time inside a spring-forward gap resolves past the gap, deterministically", () => {
      const parsed = parseCronExpression("30 2 * * *", adapter);
    ...
    test("a time earlier in the spring-forward day, before the transition itself, is unaffected", () => {
      const parsed = parseCronExpression("30 0 * * *", adapter);
...
describe("parseCronSpec", () => {
  ...
  test("an explicit * field is equivalent to omitting it", () => {
    const parsed = parseCronSpec({ minute: "*", hour: "*", dayOfMonth: "*", month: "*" }, adapter);
    const from = Date.UTC(2024, 0, 1, 10, 30, 15);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 31, 0));
  });

  test("a bare number restricts the field to a single value", () => {
    const parsed = parseCronSpec({ minute: 30, hour: 9 }, adapter);
    const from = Date.UTC(2024, 0, 1, 8, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 9, 30, 0));
  });

  test("a bare name string restricts the field to a single named value", () => {
    const parsed = parseCronSpec({ month: "JUL", dayOfMonth: 1 }, adapter);
    const from = Date.UTC(2024, 2, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 6, 1, 0, 0, 0));
  });

  test("a bare numeric string resolves the same as the equivalent number", () => {
    const parsed = parseCronSpec({ minute: "30" }, adapter);
    const from = Date.UTC(2024, 0, 1, 10, 20, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 30, 0));
  });

  test("an array of numbers is a list of allowed values", () => {
    const parsed = parseCronSpec({ minute: [0, 15, 30, 45] }, adapter);
    const from = Date.UTC(2024, 0, 1, 10, 20, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 30, 0));
  });

  test("a range object expands from-to inclusive with a default step of 1", () => {
    const parsed = parseCronSpec({ minute: [{ from: 10, to: 12 }] }, adapter);
    const from = Date.UTC(2024, 0, 1, 10, 5, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 10, 0));
  });

  test("a range object honors an explicit step", () => {
    const parsed = parseCronSpec({ minute: [{ from: 0, to: 30, step: 10 }] }, adapter);
    const from = Date.UTC(2024, 0, 1, 10, 5, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 1, 10, 10, 0));
  });

  test("a range object accepts names for from/to", () => {
    const parsed = parseCronSpec({ month: [{ from: "JAN", to: "MAR" }], dayOfMonth: 1 }, adapter);
    const from = Date.UTC(2024, 3, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2025, 0, 1, 0, 0, 0));
  });

  test("a month range accepts plain numbers for from/to", () => {
    const parsed = parseCronSpec({ month: { from: 1, to: 3 }, dayOfMonth: 1 }, adapter);
    const from = Date.UTC(2024, 3, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2025, 0, 1, 0, 0, 0));
  });

  test("a month range can mix a name for from and a number for to", () => {
    const parsed = parseCronSpec({ month: { from: "JAN", to: 3 }, dayOfMonth: 1 }, adapter);
    const from = Date.UTC(2024, 3, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2025, 0, 1, 0, 0, 0));
  });

  test("a month range can mix a number for from and a name for to", () => {
    const parsed = parseCronSpec({ month: { from: 1, to: "MAR" }, dayOfMonth: 1 }, adapter);
    const from = Date.UTC(2024, 3, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2025, 0, 1, 0, 0, 0));
  });
  ...
  test("a dayOfMonth range accepts plain numbers for from/to", () => {
    const parsed = parseCronSpec({ dayOfMonth: { from: 10, to: 15 } }, adapter);
    const from = Date.UTC(2024, 0, 20, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 1, 10, 0, 0, 0));
  });
  ...
  test("a bare range object (not wrapped in an array) is accepted directly", () => {
    ...
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 8, 1, 0, 0));
  });
  ...
  test("an array can mix numbers, names, and ranges", () => {
    ...
    const from = Date.UTC(2024, 0, 1, 10, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 3, 0, 0, 0));
  });

  test("day-of-week 7 (numeric) is normalized to the Sunday alias 0", () => {
    const parsed = parseCronSpec({ dayOfWeek: 7 }, adapter);
    // 2024-01-01 is a Monday, the next Sunday is 2024-01-07.
    const from = Date.UTC(2024, 0, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 7, 0, 0, 0));
  });
  ...
  test("day-of-week 7 inside a range is also normalized", () => {
    const parsed = parseCronSpec({ dayOfWeek: [{ from: 6, to: 7 }] }, adapter);
    // 2024-01-01 is a Monday; the nearer of the next Saturday (01-06) and Sunday (01-07) is
    // Saturday.
    const from = Date.UTC(2024, 0, 1, 0, 0, 0);
    expect(computeNextOccurrence(parsed, from, utc, adapter)).toBe(Date.UTC(2024, 0, 6, 0, 0, 0));
  });

  test("throws for a non-integer number", () => {
    expect(() => parseCronSpec({ minute: 1.5 }, adapter)).toThrow(/not a valid value/);
  });

  test("throws for a number out of range", () => {
    expect(() => parseCronSpec({ minute: 60 }, adapter)).toThrow(/out of range/);
  });
  ...
  test("throws for an unrecognized name", () => {
    ...
    expect(() => parseCronSpec({ month: "FOO" } as unknown as ICronSpec, adapter)).toThrow(
      /not a valid value/,
    );
  });

  test("throws for an empty string atom", () => {
    expect(() => parseCronSpec({ minute: "" } as unknown as ICronSpec, adapter)).toThrow(
      /not a valid value/,
    );
  });
  ...
  test.each(["0x1E", "1e1", "0o17", "0b101", "1.5"])(
    ...
    (atom) => {
      expect(() => parseCronSpec({ minute: atom } as unknown as ICronSpec, adapter)).toThrow(
        /not a valid value/,
      );
    },
  );
  ...
  test("throws for a range given in the wrong order", () => {
    expect(() => parseCronSpec({ hour: [{ from: 10, to: 5 }] }, adapter)).toThrow(/out of order/);
  });

  test("throws for a non-positive step", () => {
    expect(() => parseCronSpec({ minute: [{ from: 0, to: 10, step: 0 }] }, adapter)).toThrow(
      /not valid/,
    );
  });

  test("throws for a non-integer step", () => {
    expect(() => parseCronSpec({ minute: [{ from: 0, to: 10, step: 1.5 }] }, adapter)).toThrow(
      /not valid/,
    );
  });
});
...
describe("ICronSpec field types reject a name/value that doesn't belong to that field", () => {
  ...
  describe("numeric fields (minute/hour/dayOfMonth) - range endpoints only accept a number or a numeric string", () => {
    ...
    test("accepts a numeric string on `from`, resolving the same as the equivalent number", () => {
      const withString = parseCronSpec({ hour: { from: "1", to: 5 } }, adapter);
      const withNumber = parseCronSpec({ hour: { from: 1, to: 5 } }, adapter);
      const from = Date.UTC(2024, 0, 1, 0, 0, 0);
      expect(computeNextOccurrence(withString, from, "Etc/UTC", adapter)).toBe(
        computeNextOccurrence(withNumber, from, "Etc/UTC", adapter),
      );
    });
  ...
  describe("month field - range endpoints only accept a number or a MonthName", () => {
    ...
    test("accepts a numeric string, resolving the same as the equivalent number", () => {
      const withString = parseCronSpec({ month: { from: "1", to: 5 } }, adapter);
      const withNumber = parseCronSpec({ month: { from: 1, to: 5 } }, adapter);
      const from = Date.UTC(2024, 3, 1, 0, 0, 0);
      expect(computeNextOccurrence(withString, from, "Etc/UTC", adapter)).toBe(
        computeNextOccurrence(withNumber, from, "Etc/UTC", adapter),
      );
  ...
  describe("dayOfWeek field - range endpoints only accept a number or a DayOfWeekName", () => {
    ...
    test("accepts a numeric string, resolving the same as the equivalent number", () => {
      const withString = parseCronSpec({ dayOfWeek: { from: "1", to: 5 } }, adapter);
      const withNumber = parseCronSpec({ dayOfWeek: { from: 1, to: 5 } }, adapter);
      const from = Date.UTC(2024, 0, 1, 0, 0, 0);
      expect(computeNextOccurrence(withString, from, "Etc/UTC", adapter)).toBe(
        computeNextOccurrence(withNumber, from, "Etc/UTC", adapter),
      );
...
describe("cronExpressionToSpec", () => {
  test("represents a wildcard field as the string *", () => {
    const spec = cronExpressionToSpec("* * * * *", adapter);
    expect(spec).toEqual({
      minute: "*",
      hour: "*",
      dayOfMonth: "*",
  ...
  test("represents a restricted field as a sorted array of numbers", () => {
    const spec = cronExpressionToSpec("15,0,45,30 9 * * *", adapter);
  ...
  test("round-trips to the same next occurrence as the original expression", () => {
    ...
    const spec = cronExpressionToSpec(expression, adapter);
    const fromText = parseCronExpression(expression, adapter);
    const fromSpec = parseCronSpec(spec, adapter);
    // 2024-01-06 is a Saturday.
    const from = Date.UTC(2024, 0, 6, 10, 0, 0);
    expect(computeNextOccurrence(fromSpec, from, "Etc/UTC", adapter)).toBe(
      computeNextOccurrence(fromText, from, "Etc/UTC", adapter),
    );
  });
});
