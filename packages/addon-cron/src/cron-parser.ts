 ...
 * nothing here is a hardcoded calendar constant. A cron expression is five positional integer
 * fields; what those integers are *allowed* to be, and what names stand in for them, is the
 * calendar's business, not the parser's.
 */
...
function parsePart(part: string, bounds: FieldBounds, expression: string, into: Set<number>): void {
  /*
    Split segments are counted, not just destructured: taking the first two and dropping the rest
    would quietly read "*\/2/3" as "*\/2" and "1-2-3" as "1-2", turning a typo into a different
    but perfectly valid schedule - the same silent-misparse this file rejects elsewhere by
    refusing anything but a plain decimal integer.
  */
 ...
 * `"*\/15 8-18 * * *"`.
 *
 * Every field's valid range and accepted names come from `adapter`, so the same syntax describes
 * whatever calendar backs the runtime: `"* * 7 33 *"` is a perfectly good expression for a
 * 36-month calendar, and the names it accepts for month/day-of-week are that calendar's own
 * (`JAN`-`DEC`, `SUN`-`SAT` for the Gregorian default), matched case-insensitively.
 * @throws if `expression` doesn't have exactly 5 whitespace-separated fields, or any field is
 * malformed or out of range for `adapter`'s calendar.
 */
...
/**
 * A JSON-friendly alternative to a cron expression string - the same 5 fields a cron expression
 * encodes, addressed by name instead of position, for consumers who'd rather avoid the
 * positional/symbolic string syntax. Each field defaults to `"*"` (every value) when omitted;
...
/**
 * The structural shape every {@link ICronSpec} field value has in common, regardless of which
 * names it accepts - used internally so the parsing logic below is written once. `ICronSpec`'s
 * own per-field types are intentionally narrower for callers, to catch a field/name mismatch
 * (e.g. a day-of-week name given to `hour`) at compile time.
 */
